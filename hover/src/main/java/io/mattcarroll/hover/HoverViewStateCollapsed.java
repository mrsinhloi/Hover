...
class HoverViewStateCollapsed extends BaseHoverViewState {
    ...
    private static final String TAG = "HoverMenuViewStCollapse";
    ...
    private void listenForMenuChanges() {
        mHoverView.mMenu.setUpdatedCallback(new ListUpdateCallback() {
            ...
            public void onInserted(int position, int count) {
                // no-op
                if (mHoverView != null && mHoverView.mMenu != null) {
                    int size = mHoverView.mMenu.getSectionCount();
                    mSelectedSectionIndex += 1;

                    if (mSelectedSectionIndex >= size - 1) {
                        mSelectedSectionIndex = size - 1;
                    } else if (mSelectedSectionIndex >= size - 2) {
                        mSelectedSectionIndex = size - 2;
                    }
...
//                changeInPosition();
                    ...
                    mHoverView.setmSelectedSection(mSelectedSection);
                    //phai goi them save state moi ghi nhan
...
//                    mSelectedSectionIndex = position > 0 ? position - 1 : 0;
                ...
                /*else {
                    ...
                    if (position - 1 < mHoverView.mMenu.getSectionCount() - 1) {
                        mSelectedSectionIndex = position - 1;
                    } else {
                        mSelectedSectionIndex = mHoverView.mMenu.getSectionCount() - 1;
                    }
                    //todo Mr.Phuong add this row
                    if (mSelectedSectionIndex < 0) mSelectedSectionIndex = 0;
                    ///#
                }*/

                if (mHoverView != null && mHoverView.mMenu != null) {
                    int size = mHoverView.mMenu.getSectionCount();
                    if (mSelectedSectionIndex >= size) {//4
                        mSelectedSectionIndex = size - 1;//3
                    } else {
                        if (mSelectedSectionIndex == position) {
                            mSelectedSectionIndex = position > 0 ? position - 1 : 0;
                        }
