...
class HoverViewStateExpanded extends BaseHoverViewState {
    ...
    private int getSpacingTabview(int spaceTabview) {
        return (spaceTabview > 0) ? spaceTabview : TAB_SPACING_IN_PX;
    }
    ...
    private final Runnable mShowTabsRunnable = new Runnable() {
        @Override
        public void run() {
            if (mHoverView != null && mHoverView.mScreen != null) {
    ...
    private void createChainedTabs() {
        ...
        if (null != mHoverView.mMenu) {
            for (int i = 0; i < mHoverView.mMenu.getSectionCount(); ++i) {
                ...
                mTabChains.add(new TabChain(chainedTab, getSpacingTabview(section.getId().getSpaceTabview())));
...
//        collapse();
    ...
    private FloatingTab addTab(@NonNull HoverMenu.SectionId sectionId,
        ...
        if (mChainedTabs.size() <= position) {
            ...
            mTabChains.add(new TabChain(newTab, getSpacingTabview(sectionId.getSpaceTabview())));
        } else {
            mChainedTabs.add(position, newTab);
            mTabChains.add(position, new TabChain(newTab, getSpacingTabview(sectionId.getSpaceTabview())));
        }
    ...
    public void selectSection(int position){
        /*if(mHoverView!=null && mHoverView.mMenu!=null) {
            HoverMenu.Section section = mHoverView.mMenu.getSection(position);
            selectSection(section);
        }*/
    }
